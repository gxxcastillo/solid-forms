...
export function ThemeShowcase() {
  ...
  function onSignupSubmit(values: SignupValues) {
  ...
  // Sections are switched client-side rather than via real routes, so state
  // persists in the URL hash instead of the path — the back/forward buttons and
  // direct links (e.g. `/#login`) still work without a page navigation.
  onMount(() => {
    const fromHash = sectionFromHash(window.location.hash);
    if (fromHash) setSection(fromHash);

    const onHashChange = () => {
      // Unlike the initial read above, an in-session hash change (back/forward,
      // or the hash being cleared entirely) must also handle "no longer a known
      // section" by falling back to the default rather than leaving stale state.
  ...
  return (
    <div class={styles.page} data-sf-theme={theme()}>
      <div class={styles.inner}>
        <h1 class={styles.heading}>solid-forms demo</h1>
        <p class={styles.sub}>Theming via CSS custom properties</p>
