...
// Read-only mirror of a form's live store. Every value here comes straight off
// `form.state`, a fine-grained SolidJS store — rendering it in JSX is enough to
// make it update per keystroke, with no manual subscriptions.
        ...
        <For each={props.state.fields} fallback={<p class={styles.empty}>No fields registered yet.</p>}>
          {(field) => (
            <div class={styles.field}>
              ...
              {/* Matches the library's own getDisplayableErrors gate, so a field's errors stay hidden until it's blurred (or already valid). */}
